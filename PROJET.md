...
**Vérifier une affirmation en interrogeant la documentation qui la porte ne vérifie rien.**
Le corollaire du précédent, et il tranche un choix d'outil. « Quels fichiers construisent un
fournisseur réseau ? » se répond par l'**AST**, jamais par un `grep` : le `grep` compte les
imports, les commentaires et les docstrings — c'est-à-dire exactement les endroits où le
dépôt *parlait* du fournisseur au lieu de le construire, et où il se trompait depuis six
étapes. Un contrôle qui lit la même source que l'erreur hérite de l'erreur. La forme
...
« La base dit ce que le modèle a émis, le flux dit ce que le client a reçu — partout où l'on
re-dérive l'un depuis l'autre, on se trompe. » Elle est plus haut dans cette même section.
Elle a été écrite après l'avoir payée une fois. Et elle a été **re-violée par son auteur**,
en cherchant un message de repli dans `tours_conversation`, où il n'a par construction rien
à faire, puis en concluant de son absence une « lacune d'observation » qui n'existait pas :
l'événement était persisté depuis l'étape 23, dans `evenements_tour`.

⚠️ **La moitié utile est la cause matérielle, et elle est banale : le fil s'appelle
`fallback`, pas `repli`.** Chercher le bon concept sous le mauvais nom rend un ensemble
vide, et **un vide est indiscernable d'une absence**. Ce n'est pas la règle qui a manqué,
c'est le vocabulaire — le domaine parle français, le protocole SSE parle anglais, et la
frontière entre les deux est exactement l'endroit où une recherche échoue en silence.

La conséquence pratique tient en une ligne : **avant de conclure d'un vide, vérifier qu'on a
interrogé le bon nom.** Une requête qui ne rend rien doit d'abord se prouver capable de
rendre quelque chose — c'est « un test vert n'est pas une mesure » appliqué à une mesure
plutôt qu'à un test.
